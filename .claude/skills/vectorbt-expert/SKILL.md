...
> **Adaptation projet (ganesh_trading)** — Référence VectorBT (API `Portfolio.from_signals`, sizing, stops, walk-forward, robustesse, pièges). Les templates exécutables `rules/assets/*.py` ont été **retirés** lors de la revue de sécurité (ils lisent des clés API dans l'environnement et appellent OpenAlgo). Règles utiles ici : `us-market-costs`, `simulation-modes`, `position-sizing`, `stop-loss-take-profit`, `parameter-optimization`, `performance-analysis`, `walk-forward`, `robustness-testing`, `pitfalls`, `csv-data-resampling`, `data-fetching` (section yfinance). Ignorer : OpenAlgo, DuckDB, OpenStatz (on utilise QuantStats), coûts indiens et crypto. Indicateurs en pandas/numpy, pas `openalgo.ta`.
...
## Strategy Templates (in rules/assets/) — RETIRÉS dans ce projet

> Les fichiers `rules/assets/*.py` listés ci-dessous ont été supprimés lors de la revue de sécurité (lecture de clés API dans l'environnement, appels réseau OpenAlgo, marché indien). La table est conservée à titre documentaire ; la stratégie du projet vit dans `strategy/strategy.py`.
