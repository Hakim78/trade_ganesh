...
> **Adaptation projet (ganesh_trading)** — Le sizing du projet est une règle unique définie dans `config.yaml`, pas une décision au cas par cas. Utiliser ce skill pour **calculer et justifier** cette règle (fixed fractional / ATR / demi-Kelly) et pour vérifier le nombre d'actions QQQ/SPY passé en paper, jamais pour introduire une décision discrétionnaire dans `strategy.py`. Le script écrit dans `reports/` (ignoré par git) ; stdlib uniquement.
