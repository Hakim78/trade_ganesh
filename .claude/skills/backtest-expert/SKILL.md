...
> **Adaptation projet (ganesh_trading)** — Utiliser `scripts/evaluate_backtest.py` sur les métriques de `outputs/metrics.json` (in-sample d'abord, puis une seule fois sur l'OOS) en complément de `backtest-review`. Ses seuils (30 trades minimum, 5 ans, slippage testé) rejoignent les gates de `quant-research`. Le script écrit dans `reports/` (ignoré par git) ; stdlib uniquement.
