...
> **Adaptation projet (ganesh_trading)** — Ce skill vient de `marketcalls/vectorbt-backtesting-skills` et cible par défaut les marchés indiens (OpenAlgo). Ici : marché US (QQQ/SPY), données `yfinance` (daily/1h), pas d'OpenAlgo, pas de TA-Lib, pas de DuckDB, pas d'OpenStatz. Installer `vectorbt pandas numpy yfinance quantstats pyyaml alpaca-py python-dotenv pytest ruff` dans un venv (voir `pyproject.toml` quand il existera). Ne jamais écrire de clé API dans `.env` à la place de l'utilisateur : créer uniquement `.env.example`. Ignorer les étapes 3 (TA-Lib) et 6b à 6e (OpenAlgo / DuckDB / Crypto).
