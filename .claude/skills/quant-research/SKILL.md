...
> **Adaptation projet (ganesh_trading)** — Moteur de backtest du projet : VectorBT (`backtest/run.py`), pas ManifoldBT ; `references/manifoldbt.md` est informatif seulement. Les scripts `scripts/*.py` (numpy uniquement, sans I/O réseau) s'appliquent tels quels aux résultats VectorBT : `region_pool.py` sur un CSV de sweep, `selection_bias.py`, `sizing.py`, `effective_n.py`. Le split IS/OOS est figé dans `config.yaml` et l'hypothèse dans `docs/HYPOTHESIS.md` (Phase 0). Ne jamais toucher l'OOS avant le verdict.
