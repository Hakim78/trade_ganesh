...
> **Adaptation projet (ganesh_trading)** — Données `yfinance` QQQ/SPY : vérifier l'ajustement dividendes/splits (`auto_adjust`), la timezone (`America/New_York`), les sessions RTH (pas de barres hors séance), et que les gaps correspondent aux jours fériés NYSE. À passer avant le premier backtest (Phase 2), verdict consigné dans `docs/AUDIT.md`.
