...
> **Adaptation projet (ganesh_trading)** — Ce skill vient de `tradermonty/claude-trading-skills` et lit l'état `trader-memory-core` (fichiers YAML `th_*.yaml`), que ce projet ne produit pas. Ici, le coupe-circuit **live** est implémenté dans `live/run.py` : arrêt automatique des nouvelles entrées si le drawdown journalier dépasse le seuil de `config.yaml` (Phase 5). Utiliser ce skill comme **référence de règles** (`references/circuit_breaker_framework.md` : perte journalière max, cooldown après série perdante, limites hebdo/mensuelles, fenêtres en heure de New York) et comme modèle de sortie (`TRADING_ALLOWED / COOLDOWN / HALTED`). Le script nécessite `pandas-market-calendars` (voir `requirements.txt`) ; aucun accès réseau.
