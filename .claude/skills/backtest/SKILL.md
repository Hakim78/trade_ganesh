...
> **Adaptation projet (ganesh_trading)** — Il n'y a qu'une stratégie dans ce projet : `strategy/strategy.py::generate_signals(df)` (fonction pure, cf. `CLAUDE.md`). Ne pas générer de script `backtesting/{strategy}/…` à partir des templates `rules/assets/` (retirés). Le point d'entrée est `backtest/run.py` ; frais + slippage sont lus dans `config.yaml` (jamais les frais indiens par défaut) ; données `yfinance` ; benchmark `^GSPC` ou `QQQ` ; tearsheet **QuantStats** (`outputs/tearsheet.html`), pas OpenStatz ; indicateurs en pandas/numpy, pas `openalgo.ta`. Le reste du skill (structure d'un backtest VectorBT, `Portfolio.from_signals`, sizing, `pf.stats()`, export des trades) s'applique.
