...
> **Adaptation projet (ganesh_trading)** — Optimisation **uniquement sur l'in-sample** défini dans `config.yaml`, jamais sur l'OOS (invariant 4 de `CLAUDE.md`). Rapporter la **surface** de paramètres (heatmaps, plateau, région à 1 écart-type via `.claude/skills/quant-research/scripts/region_pool.py`), pas l'argmax : la règle du skill `quant-research` prime sur « find best parameters ». Frais/slippage depuis `config.yaml`, données `yfinance`, indicateurs pandas/numpy (pas `openalgo.ta`), pas de benchmark NIFTY.
